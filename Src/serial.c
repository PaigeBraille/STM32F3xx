...
// Default to USART3 on PB10/PB11, the original fixed assignment for this driver.
#ifndef SERIAL_PORT
...
{
    uint16_t next_head = BUFNEXT(txbuf1.head, txbuf1);  // Get pointer to next free slot in buffer
...
//
// Writes a number of characters from string to the serial1 output stream, blocks if buffer full
//
